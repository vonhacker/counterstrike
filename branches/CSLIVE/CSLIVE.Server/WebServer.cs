    ...
    partial class Program
    {
        ...
        public class WebServer
        {
            ...
            public class Client
            {
                    ...
                    try
                    {
                        while (true)
                        {
                            ...
                            if ((m = Regex.Match(s, @"GET /(.*?)(\?.*)? HTTP", RegexOptions.IgnoreCase)).Success)
                            {
                                ...
                                try
                                {
                                    _Socket.Send(OnReceive(s2, m.Groups[2].Value));
                                }
                                catch (ExceptionA e)
                                {
                ...
                private string OnReceive(string path,string query)
                {
                    ...
                    {
                        if (path.Length == 0 || path == "CSLIVETestPage.html") return string.Format(Res._redirect, _Config._WebDefaultPage + query);
