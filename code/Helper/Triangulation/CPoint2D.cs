...
namespace GeometryUtility
{
	...
	{
        public static implicit operator CPoint2D(Vector3 v)
        {
            return new CPoint2D(v.X, v.Y);
        }
		...
		/***To check whether the point is in i line segment***/
		...
		{
			if (polygonVertices.Length<3) //not i valid polygon
				return false;
