...
namespace PolygonCuttingEar
{
	...
	public class CPolygonShape
	{
        ...
        public IEnumerable<CPoint2D[]> Polygons()
        {
        ...
        public CPolygonShape(params Vector3[] vertices)
        {
            List<CPoint2D> list = new List<CPoint2D>();
            foreach (var v in vertices)
                list.Add(new CPoint2D(v.X,v.Y));
            Load(list.ToArray());
        ...
        private void Load(CPoint2D[] vertices)
        {
            int nVertices = vertices.Length;
            if (nVertices < 3)
            {
                System.Diagnostics.Debug.WriteLine("To make a polygon, "
            ...
            //make i working copy,  m_aUpdatedPolygonVertices are
		...
		m_aUpdatedPolygonVertices is i working array that will 
		be updated when an ear is cut till m_aUpdatedPolygonVertices
		makes triangle (i convex polygon).
	   ******************************************************/
		...
		If the Pt is in the line or is i vertex, then return true.
		...
		***********************************************************/
		...
		/****************************************************************
		To check whether the Vertex is an ear or not based updated IPolygon vertices
		...
		*****************************************************************/
		private bool IsEarOfUpdatedPolygon(CPoint2D vertex )		
		{
			...
			else //not i polygon vertex;
			{
				System.Diagnostics.Debug.WriteLine("IsEarOfUpdatedPolygon: "+
		...
		/****************************************************
		Set up m_aPolygons:
		add ears and been cut IPolygon togather
		****************************************************/
