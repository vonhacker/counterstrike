namespace CSLIVE.Server
{
    ...
    public partial class Program
    {
        ...
        public class GameServer
        {
            public class Room
            {
                public ContinuousCollection<Client> _Clients = new ContinuousCollection<Client>(new Client[255].ToList());
                public Room()
                {
                    _Clients.CollectionChanged += new System.Collections.Specialized.NotifyCollectionChangedEventHandler(Clients_CollectionChanged);
                }

                void Clients_CollectionChanged(object sender, System.Collections.Specialized.NotifyCollectionChangedEventArgs e)
                {
                    _RoomInfo._PlayerCount = _Clients.OfType<Client>().Count();
                }
            ...
            private void Start()
            {
                ...
                foreach (RoomInfo room in _Config.Rooms)
                {
                    _Rooms.Add(new Room { _RoomInfo = room });
                }
            ...
            public class Client : INotifyPropertyChanged
            {
                ...
                public byte? _id;
                public byte? _room;
                public ContinuousCollection<Client> _ClientsInRoom { get { return _Rooms[_room.Value]._Clients; } }
                ...
                public void Start() //intializing listener and sender
                {
                    ...
                    _Listener = new Listener() { _Socket = _Socket, _NetworkStream = _NetworkStream };
                    _Listener.StartAsync(_id.ToString());
                    _Sender = new Sender() { _Socket = _Socket, _NetworkStream = _NetworkStream };
                }
                        ...
                        if (_room == null) //client not in room
                            switch (_packet)
                            {
                                ...
                                case PacketType.getrooms: //serializing and sending rooms on server
                                    byte[] data = Common._XmlSerializerRoom.Serialize(_Rooms.Select(a => a._RoomInfo).ToList());
